...
{
	assert(length <= QUEUE_LOG_MAX_SIZE);
	memset((void*)queue_log, 0, length);
...
void LLUI_INPUT_IMPL_log_queue_add(uint32_t data, uint32_t index, uint32_t remaining_elements, uint32_t queue_length)
{
	if (queue_is_first_element)
	{
		...
		queue_log[index] = (uint8_t)(remaining_elements + 1);
	}
	else
	{
	...
	// prepare next log
	queue_is_first_element = remaining_elements == 0;
}
