...
static void IRAM_ATTR lcd_convert_frame_endianness(uint8_t *buffer, uint32_t length) {
    ...
    for (uint32_t offset = 0; offset < length; offset += (LLDISPLAY_BPP / 8)) {
...
static void IRAM_ATTR lcd_transfer_task(void * pvParameters) {
    ...
    line[0] = (uint8_t *)heap_caps_malloc(width * (LLDISPLAY_BPP / 8), MALLOC_CAP_DMA);
    line[1] = (uint8_t *)heap_caps_malloc(width * (LLDISPLAY_BPP / 8), MALLOC_CAP_DMA);
    ...
    do {
        ...
        offset = (region.ymin * width * (LLDISPLAY_BPP / 8)) + (region.xmin * (LLDISPLAY_BPP / 8));
        ...
        trans[5].length = (region.xmax - region.xmin + 1) * LLDISPLAY_BPP; // Data length, in bits
        ...
        memcpy((uint8_t *)line[sending_line], (uint8_t *)(region.srcAddr + offset), (region.xmax - region.xmin + 1) * (LLDISPLAY_BPP / 8));

        /* Convert the line frame from little-endian to big-endian if needed */
        if (!lcd_little_endian) {
            lcd_convert_frame_endianness((uint8_t *)line[sending_line], (region.xmax - region.xmin + 1) * (LLDISPLAY_BPP / 8));
        }
        ...
        for (int32_t x = region.ymin; x < (region.ymax + 1); x++) {
             ...
             */
            offset += width * (LLDISPLAY_BPP / 8);

            /* Prepare the next line to be transfered */
            memcpy((uint8_t *)line[calc_line], (uint8_t *)(region.srcAddr + offset), (region.xmax - region.xmin + 1) * (LLDISPLAY_BPP / 8));
...
void LLDISPLAY_IMPL_initialize(void)
{
    spi_bus_config_t buscfg = {
        ...
        .quadwp_io_num   = -1,
        .quadhd_io_num   = -1,
        .max_transfer_sz = DEFAULT_TFT_DISPLAY_WIDTH * DEFAULT_TFT_DISPLAY_HEIGHT * (LLDISPLAY_BPP / 8),
