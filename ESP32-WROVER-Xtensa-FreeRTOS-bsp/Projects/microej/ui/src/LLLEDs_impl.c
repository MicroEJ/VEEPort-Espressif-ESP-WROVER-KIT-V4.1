...
int32_t LLLEDS_IMPL_getIntensity(int32_t ledID)
{
	if ((ledID >= 0) && (ledID <= MAX_NUMBER_OF_LEDS)) {
		double duty = LLLEDS_MIN_INTENSITY +
				((LLLEDS_MAX_INTENSITY - LLLEDS_MIN_INTENSITY) / (MAX_LEDC_INTENSITY - MIN_LEDC_INTENSITY))
...
void LLLEDS_IMPL_setIntensity(int32_t ledID, int32_t intensity)
{
	if ((ledID >= 0) && (ledID <= MAX_NUMBER_OF_LEDS)) {
		...
		if (intensity < LLLEDS_MIN_INTENSITY) {
			intensity = LLLEDS_MIN_INTENSITY;
		}

		/* Saturate maximum intensity */
		if (intensity > LLLEDS_MAX_INTENSITY) {
			intensity = LLLEDS_MAX_INTENSITY;
		}

		/* Compute duty cycle and update it in the LEDC */
		double duty = ceil((MIN_LEDC_INTENSITY +
				((MAX_LEDC_INTENSITY - MIN_LEDC_INTENSITY) / (LLLEDS_MAX_INTENSITY - LLLEDS_MIN_INTENSITY))
				* (intensity - LLLEDS_MIN_INTENSITY)));
