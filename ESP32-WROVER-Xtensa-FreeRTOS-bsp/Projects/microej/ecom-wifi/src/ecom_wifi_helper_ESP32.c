...
void LLECOM_WIFI_init(void) {
	...
	WIFI_ESP32_initialize_f();
}

void LLECOM_WIFI_IMPL_enableSoftAP_action(MICROEJ_ASYNC_WORKER_job_t* job) {

	ECOM_WIFI_enable_join_t* param = (ECOM_WIFI_enable_join_t*) job->params;
	...
	if ((param->ssid == NULL) || (param->passphrase == NULL)) {
		...
		int8_t* ssid_buffer = param->ssid + param->ssidOffset;
		int8_t* pass_buffer = param->passphrase + param->passphraseOffset;
		bool result;
...
void LLECOM_WIFI_IMPL_setNameSoftAP_action(MICROEJ_ASYNC_WORKER_job_t* job) {
	...
	if (param->name == NULL) {
		...
		int8_t* buffer = param->name + param->nameOffset;
		bool result;
	...
	LLECOM_WIFI_DEBUG_TRACE("[%s:%u] name to set : %s (err %d)\n", __func__, __LINE__, param->name + param->nameOffset, param->error_code);
}

void LLECOM_WIFI_IMPL_getBSSID_action(MICROEJ_ASYNC_WORKER_job_t* job) {

	ECOM_WIFI_get_bssid_t* param = (ECOM_WIFI_get_bssid_t*) job->params;
	...
	if (param->bssid == NULL) {
		...
		int8_t* buffer = param->bssid + param->bssidOffset;
		bool result;

		result = WIFI_ESP32_get_bssid_f((unsigned char*)buffer, param->bssidLength);
		if (result == true) {
			param->result = 0;
		} else {
			param->result = J_EUNKNOWN;
...
void LLECOM_WIFI_IMPL_getRSSI_action(MICROEJ_ASYNC_WORKER_job_t* job) {
	...
	if (param->rssi == NULL) {
		...
		float* buffer = param->rssi + param->rssiOffset;
		bool result;
	...
	LLECOM_WIFI_DEBUG_TRACE("[%s:%u] rssi : %.2f (err %d)\n", __func__, __LINE__, *(param->rssi + param->rssiOffset), param->error_code);
}
...
void LLECOM_WIFI_IMPL_getSSID_action(MICROEJ_ASYNC_WORKER_job_t* job) {
	...
	if (param->ssid == NULL) {
		...
		int8_t* buffer = param->ssid + param->ssidOffset;
		bool result;

		result = WIFI_ESP32_get_ssid_f((char*)buffer, param->ssidLength);
		if (result == true) {
			param->result = 0;
		} else {
			param->result = J_EUNKNOWN;
	...
	LLECOM_WIFI_DEBUG_TRACE("[%s:%u] ssid : %s (err %d)\n", __func__, __LINE__, param->ssid + param->ssidOffset, param->error_code);
}
...
void LLECOM_WIFI_IMPL_scanAPCount_action(MICROEJ_ASYNC_WORKER_job_t* job) {
	...
	result = WIFI_ESP32_get_ap_count_f(&ap_count, param->active);
	if (result == true) {
...
void LLECOM_WIFI_IMPL_scanAPssidAt_action(MICROEJ_ASYNC_WORKER_job_t* job) {
	...
	if ((param->ssid == NULL) || (param->index < 0)) {
		param->result = J_EINVAL;
		if (param->error_message != NULL) {
		...
		result = WIFI_ESP32_get_ap_ssid_f(param->index, (unsigned char*)buffer, param->ssidLength);
		if (result == true) {
			param->result = 0;
		} else {
			param->result = J_EUNKNOWN;
...
void LLECOM_WIFI_IMPL_scanAPbssidAt_action(MICROEJ_ASYNC_WORKER_job_t* job) {
	...
	ECOM_WIFI_scanAP_bssid_t* param = (ECOM_WIFI_scanAP_bssid_t*) job->params;

	if ((param->bssid == NULL) || (param->index < 0)) {
		param->result = J_EINVAL;
		if (param->error_message != NULL) {
			param->error_message = "null pointer or negative index";
		}
		...
		result = WIFI_ESP32_get_ap_bssid_f(param->index, (unsigned char*)buffer, param->bssidLength);
		if (result == true) {
			param->result = 0;
		} else {
			param->result = J_EUNKNOWN;
